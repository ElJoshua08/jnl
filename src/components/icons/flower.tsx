...
export function getColorShades(hex: string): string[] {
  ...
  const lighter = adjust(originalRgb, 1.2);
  const darker1 = adjust(originalRgb, 0.85);
  const darker2 = adjust(originalRgb, 0.7);
...
export const FlowerIcon = ({
  petalColor = "#DA5A69",
  petalShadowColor = "#B3445B",
  centerColor = "#D4C790",
  centerShadowColor = "#D4C790",
  ...props
}: SVGProps<SVGSVGElement> & {
  petalColor?: string;
  petalShadowColor?: string;
  centerColor?: string;
  centerShadowColor?: string;
}) => {
  const petalColors = getColorShades(petalColor);
  const petalShadowColors = getColorShades(petalShadowColor);
